...
const Latest = () => {
  ...
  useEffect( () => {
    if ( posts.length > 0 ) {
      const filtered = posts.filter( ( post: any ) => post.status.toLowerCase() === "published" ).reverse().slice( 0, 8 );
          ...
          <div className="grid lg:grid-cols-2 md:grid-cols-1 grid-cols-1 gap-8 mt-4">
            {loading
              ? Array( 4 ).fill( 0 ).map( ( _, index ) => (
                <div key={index} className="flex flex-col gap-5 animate-pulse w-full ">
              ...
              : publishedPosts.map( ( card: any ) => (
                <div key={card._id} className="flex flex-col gap-5 cursor-pointer" onClick={() => router.push( `/detail-page/${card._id}` )}>
